  ...
  return (
    <div style={{ padding: "28px 32px", maxWidth: 1100 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: "var(--az-ink)" }}>Customers</h1>
          <p style={{ margin: "4px 0 0", fontSize: 14, color: "var(--az-muted)" }}>{SAMPLE_CUSTOMERS.length} customers · click any to create a quote</p>
        </div>
        <button style={{ padding: "10px 20px", borderRadius: 10, border: "none", background: "var(--az-navy)", color: "#ffffff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "Roboto, sans-serif" }}>
          + Add customer
        </button>
      </div>
              ...
              <tr key={c.id} style={{ borderBottom: i < filtered.length - 1 ? `1px solid ${"var(--az-line)"}` : "none" }}>
                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 34, height: 34, borderRadius: "50%", background: "var(--az-navy)", display: "flex", alignItems: "center", justifyContent: "center", color: "#ffffff", fontWeight: 700, fontSize: 13, flexShrink: 0 }}>
                      {c.name.charAt(0)}
                    </div>
                    <div>
                ...
                <td style={{ padding: "14px 16px" }}>
                  <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600, background: c.type === "Portal" ? "var(--az-blue-light)" : c.type === "Government" ? "#F0F4FF" : "var(--az-bg)", color: c.type === "Portal" ? "var(--az-blue)" : c.type === "Government" ? "#4338CA" : "var(--az-muted)" }}>
                    {c.type}
                  </span>
                </td>
                <td style={{ padding: "14px 16px", fontSize: 13, color: c.portal ? "var(--az-blue)" : "var(--az-muted)" }}>{c.portal || "—"}</td>
                ...
                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => setView("new-quote")}
                      style={{ padding: "6px 12px", borderRadius: 7, border: "none", background: "var(--az-navy)", color: "#ffffff", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "Roboto, sans-serif" }}>
                      + Quote
                    </button>
